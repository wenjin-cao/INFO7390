...
data<-read.csv("ForecastNewData2.csv",header=T)
...
DayofWeek = c()
for(i in 1:length(date)){
  DayofWeek = append(DayofWeek, ((day[i]-0)%%7)  ) 
  # if the first day is monday then -0, others -other number
}
...
write.table(data, "ready-forecast-data2.csv", sep="," ,row.name=F)
